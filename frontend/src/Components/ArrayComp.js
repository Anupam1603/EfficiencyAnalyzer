...
function ArrayComponent() {
  ...
  const handleInsert = async () => {
    try {
      const response = await axios.post('http://localhost:3000/array/insert', { value });
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/array/search/${searchValue}`);
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://localhost:3000/array/delete/${deleteValue}`);
  ...
  const handleAnalyze = async () => {
    try {
      const response = await axios.post('http://localhost:3000/array/analyze-efficiency', { dataSize: 100 });
